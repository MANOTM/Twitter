        ...
        <div className={`trends__search ${exlore && 'padding'}`}>
            <div className={`trend__header__search `}>
                <div className={`trend__search__input ${hidden || 'focus'}`}>
                    ...
                    <input type="text" defaultValue={value || hashtag} onChange={e=>{setValue(e.target.value);setHidden(false)}} autoComplete="false" onBlur={()=>setTimeout(()=>{setHidden(true)},500)} onFocus={()=>setHidden(false)}  placeholder='Search Wazoo' className='input trand__search__input' id='search' />
                </div>
